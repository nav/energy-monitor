...
mpremote connect /dev/cu.usbserial-210 fs cp src/freesans14.py :freesans14.py
...
Once WiFi connects, `energy_slides.run(oled, wri)` (called from `main.py`, reusing its existing `freesans14` `Writer` for titles rather than importing a second copy of that font) becomes the permanent home screen (it never returns). It cycles three slides every 10s (`SLIDE_INTERVAL_MS`):

1. **Demand (kW)** — current draw, big bold centered number.
2. **Demand history** — line graph of the last hour, filling the full body height; y-axis fixed at 0 on the bottom (demand doesn't go negative here, and an autoscaled floor would exaggerate small fluctuations into a chart that looks like it's swinging wildly), labeled with 0 and the window's max. No x-axis tick labels — the window is always exactly the last hour, so that's implicit rather than spelled out.
...
- `src/freesans14.py`, `src/freesans20.py` — generated fonts at 14px/20px, see "Generating a different font size" below
...
Fonts are generated from `FreeSans.ttf` (provided by `pkgs.freefont_ttf` in `shell.nix`) using `tools_font_to_py.py`:
